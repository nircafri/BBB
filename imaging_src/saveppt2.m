...
if isfield(addlParms,'ppt')
    ...
    if op==-1
       cerror('Failed invoking object. Verify that it is a PPT object as returned from a saveppt2(file,''init'') function.');
    end
...
if isfield(addlParms,'title')
    ...
    if addlParms.figure(1)==0 % Title slide only.
        ...
        % Set the alignment to left-to-right
        new_slide.Shapes.Title.TextFrame.TextRange.ParagraphFormat.Alignment='ppAlignCenter';
        %new_slide.Shapes.Title.TextFrame.TextRange.ParagraphFormat.TextDirection='ppDirectionLeftToRight';
        %new_slide.Shapes.Title.TextFrame.TextRange.ParagraphFormat.TextDirection='ppDirectionMixed';
...
for i=1:fig.count
    ...
    print(addlParms.driver,addlParms.figure(i),rendOpt,resOpt);    % Paste the contents of the Clipboard:
%    pic1 = invoke(new_slide.Shapes,'Paste');
    ...
    if checkParm(addlParms,'scale')
        ...
        if checkParm(addlParms,'stretch')
             set(pic1,'LockAspectRatio','msoTrue')
...
if isfield(addlParms,'text')
    ...
    set(new_text.TextFrame.TextRange.Font,'Size',24)    
    set(new_text.TextFrame.TextRange.ParagraphFormat,'TextDirection','ppDirectionLeftToRight')    
end
%% Exit Functions
